...
## 2026-03-27 FORMAT_REVERT_PHASE2_SAFE: Template Cleanup

**ROLE:** DEV

**CONTEXT:**
Phase 2 safe subset — перевести ROLE_COST/ROLE_DECISION в bullet format и убрать box drawing из документации.

**CHANGES:**
- ROLE_COST: table → bullet list
- ROLE_DECISION: table → bullet list
- Decision Flow Diagram: box drawing → text description
- AUTO_NEXT_TASK: tree arrows → nested bullets
- GIT_WORKFLOW: box drawing → text checklist

**FILES:**
- `AGENT_POLICY.md` — templates cleaned

**STATUS:** PHASE2_SAFE COMPLETE ✅

**Removed:**
- Markdown tables in ROLE_COST, ROLE_DECISION
- Box drawing (┌─│└─) from Decision Flow, GIT_WORKFLOW
- Tree arrows (├─└─) from AUTO_NEXT_TASK

**Preserved:**
- Section dividers (---)
- OWNER_SUMMARY format
- NEXT TASK SUGGESTION format

---

## 2026-03-27 TOKEN_OBSERVABILITY: Lightweight Token Tracking

**ROLE:** PM

**CONTEXT:**
Нет видимости расхода токенов на pipeline этапы, но сложная система анализа не нужна.

**PROBLEM:**
- Непонятно где теряются токены
- Нет данных для оптимизации
- Сложный анализ требует новой роли

**DECISION:**
- Добавить lightweight TOKEN_USAGE блок в OWNER_SUMMARY
- НЕ создавать новую роль — использовать TEST или PM
- НЕ делать сложные вычисления — только approximate
- НЕ вызывать дополнительные модели

**CHANGES:**
- AGENT_POLICY.md — Section 8: TOKEN_OBSERVABILITY added
- Version: 1.10 → 1.11
...
**Integration:**
- Где: конец OWNER_SUMMARY
- Кто: TEST или PM

---

## 2026-03-27 VERBOSITY_CONTROL: Strict Brevity Rules

**ROLE:** PM

**CONTEXT:**
Pipeline генерирует избыточные сообщения → рост токенов и шум в Telegram.

**PROBLEM:**
- Длинные ответы без запроса
- Повторение контекста
- "Объяснения ради объяснений"
- Рост token usage

**DECISION:**
- Строгие лимиты на длину ответов для каждой роли
- Приоритет: краткость > полнота
- Запретить пересказ предыдущих шагов и повтор контекста

**CHANGES:**
- AGENT_POLICY.md — Section 9: VERBOSITY_CONTROL added
- Version: 1.11 → 1.12
...
## 2026-03-27 CONTEXT_PRUNING: Minimal Context Between Steps

**ROLE:** PM

**CONTEXT:**
Контекст растёт между шагами pipeline → лишние prompt_tokens и рост стоимости.

**PROBLEM:**
- Передаётся полная история между stages
- Старые OWNER_SUMMARY дублируются
- Контекст раздувается без пользы

**DECISION:**
- MAX_CONTEXT_BLOCKS = 3 (task, last_result, markers)
- Удалять старые обсуждения и предыдущие summaries
- Приоритет: актуальность > полнота
- Fallback: если > лимита → оставить только task + last_result + markers

**CHANGES:**
- AGENT_POLICY.md — Section 10: CONTEXT_PRUNING added
- Version: 1.12 → 1.13
...
**Expected token reduction:** 30-50% на handoff

---

## 2026-03-27 POLICY_ENFORCEMENT: TEST Validates Compliance

**ROLE:** PM

**CONTEXT:**
Policy может дрейфовать — роли иногда нарушают правила без последствий.

**PROBLEM:**
- VERBOSITY_CONTROL игнорируется
- Markdown tables появляются снова
- CONTEXT_POLICY не соблюдается
- READY_FOR_TEST маркер забывается

**DECISION:**
- TEST роль дополнительно проверяет policy compliance
- FAIL при любом нарушении
- OWNER_SUMMARY включает POLICY_STATUS блок
- НЕ вызывать дополнительные модели — только output проверка

**CHANGES:**
- AGENT_POLICY.md — Section 11: POLICY_ENFORCEMENT added
- Version: 1.13 → 1.14
...
| Check | Что проверять |
|-------|---------------|
| VERBOSITY_CONTROL | Лимиты строк |
| FORMAT_POLICY | Нет tables/pseudographics |
| CONTEXT_POLICY | MAX_CONTEXT_BLOCKS=3 |
| HANDOFF_RULES | READY_FOR_TEST маркер |

**Violation Types:**
- verbosity_exceeded
- markdown_table_found
- context_blocks_exceeded
- missing_READY_FOR_TEST
...
## 2026-03-27 PIPELINE_WATCHDOG: PM Monitors for Stalls

**ROLE:** PM

**CONTEXT:**
Pipeline может зависнуть в TUI, а в Telegram это не видно пользователю.

**PROBLEM:**
- Нет видимости зависаний между stages
- Пользователь не знает о проблемах
- Recovery требует ручного вмешательства

**DECISION:**
- PM отслеживает выполнение pipeline
- STALL detection: 10 сек между stages
- Auto-recovery при зависании
- Owner alert при критических проблемах

**CHANGES:**
- AGENT_POLICY.md — Section 13: PIPELINE_WATCHDOG added
- Version: 1.15 → 1.16
...
**PM Responsibilities:**
1. Отслеживать зависания между stages
2. Проверять запуск следующего subagent
3. Пытаться восстановить автоматически
4. Сообщать владельцу при проблемах

**Stall Detection:**
- Timeout: 10 сек между stages
- Ожидаемые переходы: PM→DEV→TEST→OWNER_SUMMARY→NEXT TASK
...
## 2026-03-27 LANGUAGE_POLICY: Russian for User-Facing Content

**ROLE:** PM

**CONTEXT:**
Описание задач и сообщения начинают переходить на английский → ухудшается читаемость для русскоязычной команды.

**PROBLEM:**
- Задачи генерируются на английском
- OWNER_SUMMARY смешивает языки
- DEVLOG записи на английском
- Ухудшается читаемость и понимание

**DECISION:**
- Обязательный русский для user-facing контента
- Английский разрешён только для кода/терминов
- Auto-correction: английский → русский перевод
- Запретить смешивание языков

**CHANGES:**
- AGENT_POLICY.md — Section 12: LANGUAGE_POLICY added
- Version: 1.14 → 1.15
